(function () {
 ...
 function render(){var snap=table.snapshot(),ev=current(),keys=visibleBoard.length>=3?ev.bestCards.map(Poker.cardKey):[];
  ...
  for(var i=1;i<table.players.length;i++){var p=snap.players[i],v=visibleSeats[i],seat=$('seat'+i),mood=started?(v.folded?'folded':v.allIn?'allin':'playing'):'playing';
   // p.connected only exists online (see worker/room.js's _decorate) - an AI
   // companion seat never carries it, so this never fires in single-player.
   var away=online&&p.connected===false;
   seat.querySelector('.stack').textContent=v.stack.toLocaleString();seat.querySelector('.status').textContent=!started?'Ready':away?(activeSeat===i&&!revealed?'Away · their turn':'Away'):(activeSeat===i&&!revealed?'Their turn':v.lastAction||(v.folded?'Resting':''));seat.querySelector('.dealer-token').style.display=started&&snap.dealer===i?'block':'none';seat.classList.toggle('active',started&&activeSeat===i&&!revealed);seat.querySelector('.nameplate').setAttribute('aria-current',activeSeat===i?'true':'false');HearthPresentation.renderChips(seat.querySelector('.npc-chips'),v.stack,names[i]);seat.classList.toggle('folded',started&&v.folded);seat.classList.toggle('away',away);seat.dataset.mood=mood;var sprite=seat.querySelector('.sprite');sprite.setAttribute('role','img');sprite.setAttribute('aria-label',names[i]+(away?', away from the table':'')+', '+(mood==='allin'?'all in':mood==='folded'?'sitting this hand out':'playing'));replaceCards(seat.querySelector('.opponent-cards'),started?p.hole.map(function(c){return revealed?c:null;}):[null,null],{animate:true});}
 ...
 // A small cropped headshot from the same seated sprite sheet the table
 // already draws (asset lookup mirrors roster.js's mount()), so the recap
 ...
 function renderRecap(){var busted=table.players[0].stack===0;$('recapNextHand').textContent=eveningOver()?'Settle up ↗':busted?'Rebuy '+table.startingStack+' chips ↗':'Deal next hand ↗';var target=$('recapContent');target.innerHTML='';target.scrollTop=0;var result=table.result;
  if(result.showdown&&result.showdown.length){var showdownSection=document.createElement('section');showdownSection.className='recap-showdown';var heading=document.createElement('h3');heading.textContent='Showdown hands';showdownSection.appendChild(heading);var winnerIds=result.winners.filter(function(w){return w.wonAmount>0;}).map(function(w){return w.id;});result.showdown.slice().sort(function(a,b){return Poker.compare(b.hand,a.hand);}).forEach(function(p){var row=document.createElement('div');row.className='recap-hand-row'+(winnerIds.indexOf(p.id)>=0?' winner':'');var pname=p.id===0?'You':names[p.id];row.appendChild(portraitFor(pname));var name=document.createElement('span');name.className='recap-hand-name';name.textContent=pname;row.appendChild(name);row.appendChild(miniCards(p.hole));var label=document.createElement('span');label.className='recap-hand-label';label.textContent=p.hand.name;row.appendChild(label);showdownSection.appendChild(row);});target.appendChild(showdownSection);}
  HearthSession.recap(result,names).forEach(function(pot){var section=document.createElement('section');var heading2=document.createElement('h3');heading2.textContent=pot.title;section.appendChild(heading2);pot.winners.forEach(function(w){var row=document.createElement('div');row.className='recap-pot-row';row.appendChild(portraitFor(w.name));var name=document.createElement('strong');name.textContent=w.name+(w.hand?' · '+w.hand:'')+' · '+w.amount+' chips';row.appendChild(name);row.appendChild(miniCards(w.cards));section.appendChild(row);});var detail=document.createElement('p');detail.textContent=pot.text;section.appendChild(detail);target.appendChild(section);});}
 ...
 function renderOnlineLobby(msg){
  ...
  if(onlineRoomCode){var copyBtn=document.createElement('button');copyBtn.id='onlineCopyCode';copyBtn.type='button';copyBtn.className='subtle-button';copyBtn.textContent='Copy';codeLine.appendChild(copyBtn);}
 ...
 // execCommand('copy') is deprecated but synchronous, so it still runs
 // inside this click's own user-activation window and needs no permission
 // grant - unlike navigator.clipboard.writeText, whose async .catch() can
 // arrive after that window has closed, silently failing with no feedback.
 // It also works on the Windows shell's file:// page, which never counts as
 // a secure context. So it's tried first; the Clipboard API is only a
 // best-effort fallback for a browser that has dropped execCommand entirely.
 function onlineCopyCode(){
  var code=onlineRoomCode?HearthRoomCode.display(onlineRoomCode):'';if(!code)return;
  var btn=$('onlineCopyCode');
  function copied(){if(!btn)return;btn.textContent='Copied!';setTimeout(function(){if(btn.isConnected)btn.textContent='Copy';},1500);}
  var ok=false;
  try{var ta=document.createElement('textarea');ta.value=code;ta.style.position='fixed';ta.style.opacity='0';document.body.appendChild(ta);ta.focus();ta.select();ok=document.execCommand('copy');document.body.removeChild(ta);}catch(e){}
  if(ok){copied();return;}
